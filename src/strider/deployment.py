...
from strider.evaluation.evaluate import _posterior_basin_candidates
from strider.model import Strider, measurement_inputs
from strider.model_package import (
    CALIBRATION_FORMAT,
    MODEL_PACKAGE_FORMAT,
    model_package_asset_specs,
)
...
def load_model_package(
    ...
    *,
    ...
    missing = [
        name
        for name in _BASE_REQUIRED_PACKAGE_FILES
    ...
    asset_files = tuple(
        package_name
        for _, _, package_name in model_package_asset_specs(config)
    )
    required_files = (*_BASE_REQUIRED_PACKAGE_FILES, *asset_files)
    missing_assets = [
        name for name in asset_files if not (package_dir / name).is_file()
    ]
    if missing_assets:
        raise FileNotFoundError(
            f"Model package {package_dir} is missing: {', '.join(missing_assets)}"
        )
    if verify_checksums:
        _verify_checksums(package_dir, required_files)
    ...
    recorded_architecture = model_info.get("architecture")
    if (
        recorded_architecture is not None
        and str(recorded_architecture) != architecture
    ):
        raise ValueError("model_info.json architecture does not match the configuration")
    recorded_assets = model_info.get("model_assets")
    if recorded_assets is not None and list(recorded_assets) != list(asset_files):
        raise ValueError("model_info.json assets do not match the model architecture")
    # Package-v1 did not originally record these descriptive fields. The
    # resolved configuration remains authoritative for those older packages.
    model_info.setdefault("architecture", architecture)
    model_info.setdefault("model_assets", list(asset_files))
...
def prepare_observed_series(
    ...
    *,
    ...
    coverage: list[float] = []
    use_error_shape = bool(
        config["data"].get("include_flux_error_channel", False)
        or config["model"].get("use_flux_error_channel", False)
    )
    for visit_index, (visit_flux, visit_error) in enumerate(
        ...
        if use_error_shape:
            ...
            target_wave = output_wave[inside]
            upper = np.searchsorted(native_wave, target_wave, side="left")
            upper = np.clip(upper, 1, len(native_wave) - 1)
            lower = upper - 1
            denominator = native_wave[upper] - native_wave[lower]
            fraction = np.divide(
                target_wave - native_wave[lower],
                denominator,
                out=np.zeros_like(target_wave, dtype=np.float64),
                where=denominator > 0.0,
            )
            scaled_error = native_error / scale
            propagated_variance = (
                np.square(1.0 - fraction) * np.square(scaled_error[lower])
                + np.square(fraction) * np.square(scaled_error[upper])
            )
            propagated_error = np.sqrt(propagated_variance)
            valid_error = (
                np.isfinite(propagated_error)
                & (propagated_error > 0.0)
                & np.isfinite(denominator)
                & (denominator > 0.0)
            )
            inside_indices = np.flatnonzero(inside)
            valid_indices = inside_indices[valid_error]
            model_error[valid_indices] = np.log(
                propagated_error[valid_error]
            ).astype(np.float32)
            inside[inside_indices[~valid_error]] = False
            model_flux[~inside] = 0.0
            resampled_mask[-1] = inside.astype(np.float32)
...
def _public_result(
    ...
    *,
    ...
    candidates = _posterior_basin_candidates(grid, redshift_probability, widths)
    ...
    result: dict[str, Any] = {
        ...
        "model": {
            ...
            "architecture": deployed.model_info["architecture"],
...
def _runtime_config(config: dict[str, Any], package_dir: Path) -> dict[str, Any]:
    ...
    for section, key, package_name in model_package_asset_specs(runtime):
        runtime.setdefault(section, {})[key] = package_name
    return runtime
...
def _deployment_wavelength_mask(
    ...
    *,
    ...
    if template_support_policy(observation) != "complete":

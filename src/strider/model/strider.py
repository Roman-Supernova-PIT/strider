...
from .coadd import final_equal_weight_coadd, final_inverse_variance_coadd
...
from .roman_reference import RomanReferenceEvidence
...
class Strider(nn.Module):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.coadd_reconstruction_head: nn.Module | None = None
        self.roman_reference: RomanReferenceEvidence | None = None
        ...
        self.dense_scan_input_mode = str(
            model.get("dense_scan_input_mode", "individual_visits")
        )
        if self.dense_scan_input_mode not in {
            "individual_visits",
            "inverse_variance_coadd",
        }:
            raise ValueError(
                "dense_scan_input_mode must be 'individual_visits' or "
                "'inverse_variance_coadd'"
            )
        configured_coadd_error_limit = model.get(
            "coadd_maximum_relative_error", 3.0
        )
        self.coadd_maximum_relative_error = (
            None
            if configured_coadd_error_limit is None
            else float(configured_coadd_error_limit)
        )
        if (
            self.coadd_maximum_relative_error is not None
            and self.coadd_maximum_relative_error <= 0.0
        ):
            raise ValueError("coadd_maximum_relative_error must be positive")
        coadd_edge_trim_fraction = float(
            model.get("coadd_edge_trim_fraction", 0.05)
        )
        if not 0.0 <= coadd_edge_trim_fraction < 0.5:
            raise ValueError("coadd_edge_trim_fraction must lie in [0, 0.5)")
        log_coordinate = (
            np.log(observed_wavelength) - np.log(observed_wavelength[0])
        ) / np.log(observed_wavelength[-1] / observed_wavelength[0])
        self.register_buffer(
            "coadd_edge_mask",
            torch.from_numpy(
                (
                    (log_coordinate >= coadd_edge_trim_fraction)
                    & (log_coordinate <= 1.0 - coadd_edge_trim_fraction)
                ).astype(np.float32)
            ),
            persistent=False,
        )
        self.coadd_weighting = str(model.get("coadd_weighting", "reported_error"))
        if self.coadd_weighting not in {
            "automatic",
            "reported_error",
            "equal",
        }:
            raise ValueError(
                "coadd_weighting must be 'automatic', 'reported_error', or 'equal'"
            )
        coadd_reconstruction = bool(model.get("coadd_reconstruction", False))
        if self.dense_scan_input_mode == "inverse_variance_coadd":
            if not bool(model.get("dense_rest_frame_scan", False)):
                raise ValueError(
                    "inverse_variance_coadd requires dense_rest_frame_scan: true"
                )
            if not bool(config["data"].get("include_flux_error_channel", False)):
                raise ValueError(
                    "inverse_variance_coadd requires "
                    "data.include_flux_error_channel: true"
                )
            if self.use_flux_error_channel:
                raise ValueError(
                    "The coadd uncertainty must weight measurements, not be "
                    "added to flux through use_flux_error_channel"
                )
        elif coadd_reconstruction:
            raise ValueError(
                "coadd_reconstruction requires dense_scan_input_mode: "
                "inverse_variance_coadd"
            )
        if coadd_reconstruction and not bool(
            config["data"].get("include_clean_flux_target", False)
        ):
            raise ValueError(
                "coadd_reconstruction requires data.include_clean_flux_target: true"
            )
        ...
        if self.architecture == "roman_reference":
            if self.coadd_weighting == "reported_error" and not bool(
                config["data"].get("include_flux_error_channel", False)
            ):
                raise ValueError(
                    "Roman reference matching with reported_error weighting "
                    "requires reported flux errors"
                )
            if self.use_flux_error_channel:
                raise ValueError(
                    "Roman reference matching uses uncertainty for weighting, "
                    "not as an added flux channel"
                )
            ...
            if self.architecture == "factored_onir":
                ...
                if bool(model.get("dense_rest_frame_scan", False)):
                    ...
                    if coadd_reconstruction:
                        hidden = self.onir.temporal_representation_dim
                        self.coadd_reconstruction_head = nn.Sequential(
                            nn.LayerNorm(hidden),
                            nn.Linear(hidden, 1),
                        )
        ...
        elif self.architecture == "roman_reference":
            self.onir = None
            self.scan = None
            self.temporal_mode = "reference_sequence"
            self.roman_reference = RomanReferenceEvidence(
                config,
                observed_wavelength,
                redshift_grid,
            )
    ...
    def _coadded_measurement(
        self, batch: dict[str, torch.Tensor]
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Construct one deployable coadd without simulation-only inputs."""
        use_reported_error = self.coadd_weighting == "reported_error" or (
            self.coadd_weighting == "automatic" and "flux_error_shape" in batch
        )
        if use_reported_error and "flux_error_shape" not in batch:
            raise KeyError(
                "Reported-error accumulation requires flux_error_shape"
            )
        visit_scale = batch.get("visit_flux_scale")
        if visit_scale is None:
            visit_scale = torch.ones_like(batch["visit_mask"], dtype=batch["flux"].dtype)
        if use_reported_error:
            flux, error, mask = final_inverse_variance_coadd(
                batch["flux"],
                batch["wavelength_mask"],
                batch["visit_mask"],
                batch["flux_error_shape"],
                visit_scale,
                maximum_relative_error=self.coadd_maximum_relative_error,
            )
        else:
            flux, mask = final_equal_weight_coadd(
                batch["flux"],
                batch["wavelength_mask"],
                batch["visit_mask"],
                visit_scale,
            )
            error = torch.zeros_like(flux)
        mask = mask * self.coadd_edge_mask[None, :].to(mask.dtype)
        error_flag = torch.full(
            (flux.shape[0],),
            float(use_reported_error),
            dtype=flux.dtype,
            device=flux.device,
        )
        return flux * mask, error * mask, mask, error_flag
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if self.architecture == "roman_reference":
            if self.roman_reference is None:
                raise RuntimeError("Roman reference matcher is missing")
            coadded_flux, coadded_error, coadded_mask, used_reported_error = (
                self._coadded_measurement(batch)
            )
            output = self.roman_reference(
                batch,
                coadded_flux,
                coadded_mask,
                coadded_error,
            )
            output["coadded_valid_fraction"] = coadded_mask.mean(dim=-1)
            output["coadd_used_reported_error"] = used_reported_error
            output["evidence_sufficiency_logit"] = self.evidence_sufficiency(
                batch
            )
            return output
        if self.architecture in {"onir", "encoded_onir", "factored_onir"}:
            ...
            if self.factored_evidence is not None:
                ...
                if self.dense_scan is not None:
                    ...
                    dense_tokens = observer_tokens
                    dense_token_mask = observer_token_mask
                    dense_visit_mask = batch["visit_mask"]
                    if self.dense_scan_input_mode == "inverse_variance_coadd":
                        coadded_flux, _, coadded_mask, _ = (
                            self._coadded_measurement(batch)
                        )
                        dense_tokens, dense_token_mask = self.onir.encode_observer_flux(
                            coadded_flux[:, None, :],
                            coadded_mask[:, None, :],
                            normalize=True,
                        )
                        dense_visit_mask = coadded_mask.any(dim=-1).to(
                            coadded_flux.dtype
                        )[:, None]
                        output["coadded_valid_fraction"] = coadded_mask.mean(dim=-1)
                        if self.coadd_reconstruction_head is not None:
                            reconstruction = self.coadd_reconstruction_head(
                                dense_tokens[:, 0]
                            ).squeeze(-1)
                            output["coadd_reconstruction"] = (
                                reconstruction * coadded_mask
                            )
                            output["coadd_reconstruction_mask"] = coadded_mask

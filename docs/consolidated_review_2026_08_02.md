...
| E7 | Front-end nonlinearity | Not considered a design variable | **Treat as leak-relevant.** | ADD | On a source-deleted probe the GELU conv stem reached **96%** of the analytic oracle; linear patch projection reached **37%**. A rectifying front end converts local noise amplitude into a positive feature; a zero-mean NCC match cannot. Independently reproduced in two analogues. | RESOLVED, SYNTHETIC |
...
| A6 | `overlap_norm` | Implemented, tried, "blew up mid-z aliasing" | **Do not adopt as specified in the audit.** Use the log-coverage term instead. | FIX | **Root cause (VERIFIED):** `edna_detector.py:262` uses `gather_valid.any(dim=-1)`, so a feature clinging to the band by **1 patch of 15 gets full weight**. That is a sharper explanation than "it deleted the coverage prior", and the recommendation stands on this alone. The analogue's reproduction of the failure is **UNDERPOWERED** (f1 Δ=−0.041 [−0.063,−0.019] against a 0.020 margin, n_req=22) — directionally consistent, not resolved. | **VERIFIED (root cause); UNDERPOWERED (reproduction)** |
...
### 2.8 Software, config, reproducibility
...
| **X2** | **Reproduce the shipped checkpoint from source.** Rebuild `strider_15class.pt` from a repo config at a known commit; diff the state dict. | 1 day, no GPU | Rewrite vs refactor, empirically. Also recovers (or definitively loses) provenance given `training_config_hash=''` and `36717bb-dirty`. Start regardless of sequence. |
...
| "Nothing is reproducible" (Risk 5) | Overstated - the detector spec exists and is correct in `train_15class.yaml`. The defect is a dead block that *looks* live |
...
| A6 `overlap_norm` reproduction tagged RESOLVED | **UNDERPOWERED** (n_req=22). Root cause at `edna_detector.py:262` is VERIFIED and carries the recommendation alone |
